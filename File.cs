            ...
            switch (args[0])
            {
                ...
                case "createdir":
                    try
                    {
                        ...
                        response = "Sucessfully created directoory" + args[1];
                    }
                    catch (Exception ex)
                    {
                        response = ex.ToString();
                ...
                case "deldir":
                    try
                    {
                        ...
                        response = "Sucessfully remove directoory" + args[1];
                    }
                    catch (Exception ex)
                    {
                        response = ex.ToString();
                    ...
                    try
                    {
                       if (args[1].Equals("-e"))
                        ...
                        else if (args[1].Equals("-o"))
                        {
                            ...
                            Sys.FileSystem.VFS.VFSManager.DeleteFile(name1);
                            Sys.FileSystem.VFS.VFSManager.CreateFile(name1);
                            Console.WriteLine("Enter what to Write in the text file and to go to next line type \"\\n\" preceeded and followed by a \' \'(space) : \n");
                            String s = Console.ReadLine();
                            string[] str = s.Split(" ");
                            FileStream fs = (FileStream)Sys.FileSystem.VFS.VFSManager.GetFile(name1).GetFileStream();
                            change(str, fs);
                ...
                case "dirlist":
                    ...
                    Console.WriteLine("Name\t\t\t\t\tSize(number of characters)");
                    foreach (var directory in directory_list)
                    {
                        if (directory.Equals("0:\\Pass123"))
                        {
                            continue;
                        }
                ...
                case "copy":
                    try
                    {
                        ...
                        if (f == 1)
                        {
                            Console.Write("File Already Presen. Do you want to overwrite any data that is there in the file?(y/n)");
                            String s1 = Console.ReadLine();
                            while (true)
                            {
